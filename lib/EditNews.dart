...
class EditNewsState extends State<EditNews> {
  ...
  addDataFromModel() {
    Future.delayed(Duration.zero, () {
      ...
      if (conTypeId == "video_youtube" ||
          conTypeId == "video_other" ||
          conTypeId == "video_upload") {
...
// set image gallery
            ...
            child: image != null
                ...
                : CachedNetworkImage(
                    ...
                    errorWidget: (context, error, stackTrace) {
                      return errorWidget(125, double.infinity);
                    },
                  )),
      ),
    );
  ...
  Widget modelOtherImage() {
    print("other image list len****${widget.model.imageDataList!.length}");
                    ...
                    return Container(
                      ...
                      child: Stack(
                        children: [
                          Padding(
                            padding:
                                EdgeInsetsDirectional.only(top: 10, end: 8),
                            child: ClipRRect(
                                borderRadius: BorderRadius.circular(10.0),
  ...
  deleteImageId(String id, int index) async {
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      var param = {ACCESS_KEY: access_key, ID: id};
      Response response = await post(Uri.parse(setDeleteImageApi),
      ...
      if (error == "false") {
        setState(() {
          widget.model.imageDataList!.removeAt(index);
        });
    ...
    return Padding(
      ...
      child: InkWell(
          ...
          onTap: () async {
            ...
            if (form.validate()) {
              if (conType == getTranslated(context, 'VIDEO_UPLOAD_LBL')!) {
                if (videoUpload == null) {
                  if (urlC.text.isEmpty) {
                    showSnackBar(
                        getTranslated(context, 'PLZ_UPLOAD_VIDEO_LBL')!,
                        context);
                  } else {
                    setState(() {
                      isNext = true;
  ...
  Future<void> editNewsAdd() async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        var request = MultipartRequest('POST', Uri.parse(updateNewsApi));
        request.headers.addAll(headers);

        request.fields[USER_ID] = CUR_USERID;
        request.fields[NEWS_ID] = widget.model.id!;
        ...
        if (urlC.text != widget.model.contentValue) {
          if (conType == getTranslated(context, 'VIDEO_YOUTUBE_LBL')!) {
